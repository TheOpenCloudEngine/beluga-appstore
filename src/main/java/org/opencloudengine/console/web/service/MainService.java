package org.opencloudengine.console.web.service;
...
import org.opencloudengine.console.web.db.dao.TenantMapper;
import org.opencloudengine.console.web.db.dao.TenantUserMapper;
import org.opencloudengine.console.web.db.dao.UserMapper;
import org.opencloudengine.console.web.db.entity.Tenant;
import org.opencloudengine.console.web.db.entity.TenantUser;
import org.opencloudengine.console.web.db.entity.User;
...
public class MainService {
	 ...
	 * 테넌트 관리자 회원가입.
	 * */
	public void signInAdmin(Tenant tenant, User user) throws Exception {
		Tenant prevTenant = tenantMapper.select(tenant.getId());
		//테넌트 이미 존재시 에러. Admin으로 가입불가.
		if(prevTenant != null) {
			throw new RuntimeException("Tenant already exists : " + tenant.getId());
		}

		User prevUser = userMapper.selectById(tenant.getId(), user.getId());
		//테넌트에 동일 사용자 존재시 에러.
		if(prevUser != null) {
			throw new RuntimeException("User already exists : " + tenant.getId() + "/" + user.getId());
		}
		TenantUser prevTenantUser = tenantUserMapper.select(tenant.getId(), user.getId());
		//테넌트-사용자 매핑에 동일 사용자 존재시 에러.
		if(prevTenantUser != null) {
			throw new RuntimeException("Tenant-User already exists : " + tenant.getId() + "/" + user.getId());
		}

		TenantUser tenantUser = new TenantUser(tenant.getId(), user.getId());
		//1.테넌트추가.
	...
	public void signInUser(String tenantId, User user) throws Exception {
		Tenant prevTenant = tenantMapper.select(tenantId);
...
//			status = tenantSubscribe(domain, appId, planId, user.getTenantId());
